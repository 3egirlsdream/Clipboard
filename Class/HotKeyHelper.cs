...
namespace Clipboards.Class
{
    ...
    {
        public static bool IsIgnorance = false;
        /// <summary>
        /// 记录快捷键注册项的唯一标识符
        /// </summary>
        ...
        public static string RegisterGlobalHotKey(IEnumerable<HotKeyModel> hotKeyModelList, IntPtr hwnd, out Dictionary<EHotKeySetting, int> hotKeySettingsDic)
        {
            ...
            foreach (var item in hotKeyModelList)
            {
                if (!RegisterHotKey(item, hwnd))
                {
                    ...
                    else if(item.IsSelectWin && !item.IsSelectAlt && !item.IsSelectCtrl && !item.IsSelectShift)
                    {
                        str = ModifierKeys.Windows.ToString();
                    }
        ...
        private static bool RegisterHotKey(HotKeyModel hotKeyModel, IntPtr hWnd)
        {
            ...
            else if(!hotKeyModel.IsSelectCtrl && !hotKeyModel.IsSelectShift && !hotKeyModel.IsSelectAlt && hotKeyModel.IsSelectWin)
            {
            ...
            var f = HotKeyManager.RegisterHotKey(hWnd, m_HotKeySettingsDic[hotKeySetting], fsModifierKey, (int)hotKeyModel.SelectKey);
            var errorcode = Marshal.GetLastWin32Error();
            var error = Marshal.GetExceptionForHR(errorcode);
            return f;
        }
